...
{
  ${script:ERROR_LABEL} = "[${SCRIPT}] ERROR:"
  ${script:WARNING_LABEL} = "[${SCRIPT}] WARNING:"
  ${script:INFO_LABEL} = "[${SCRIPT}] INFO:"
}
else
{
  ${script:ESC} = ''
  ${script:RED} = "${ESC}[1;31m"
  ${script:YELLOW} = "${ESC}[1;33m"
  ${script:GREEN} = "${ESC}[1;32m"
  ${script:STOP_COLOR} = "${ESC}[0m"
  ${script:ERROR_LABEL} = "[${SCRIPT}] ${RED}ERROR${STOP_COLOR}:"
  ${script:WARNING_LABEL} = "[${SCRIPT}] ${YELLOW}WARNING${STOP_COLOR}:"
  ${script:INFO_LABEL} = "[${SCRIPT}] ${GREEN}INFO${STOP_COLOR}:"
}
...
################################################################################
    ${homePath} = if (${IsWindows} -or ${env:OS} -match 'Windows') { ${env:USERPROFILE} } else { ${env:HOME} }
...
################################################################################
# Writing back the cleaned lines to the file.                                  #
################################################################################
    Set-Content ${XmlFile} -Value ${lines} -ErrorAction Stop

################################################################################
# Adding a single blank line at the end of the file.                           #
################################################################################
    Add-Content ${XmlFile} -Value ''
  }
  catch
  {
    Write-Error "${ERROR_LABEL} [$(${MyInvocation}.MyCommand.Name)] Failed to reduce trailing blank lines into a single blank line in [${FileType}] file [$(Sanitize-Path -FilePath ${XmlFile})] due to [$(${_}.GetType().FullName)]: $(${_}.Exception.Message)"
    return 1
  }
  return 0
}
...
################################################################################
    ...
    ${stat} = Post-Beautify-XML -XmlFile ${XmlFile} -FileType ${FileType}
    if (0 -eq ${stat})
    {
...
################################################################################
    ...
    {
      Write-Information "${INFO_LABEL} [$(${MyInvocation}.MyCommand.Name)] The XML file [$(Sanitize-Path -FilePath ${XmlFile})] already contains a DOCTYPE declaration."
