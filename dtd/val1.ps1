...
$handler = [System.Xml.Schema.ValidationEventHandler]{
    ...
    $errors += [PSCustomObject]@{
        Severity = $e.Severity
        Message  = $e.Message
        Node     = if ($sender -is [System.Xml.XmlNode]) {
            $sender.OuterXml.Substring(0, [Math]::Min(80, $sender.OuterXml.Length))
        } else {
            "<unknown>"
        }
    }
...
if ($errors.Count -eq 0) {
    Write-Host "XML is valid against schema." -ForegroundColor Green
} else {
