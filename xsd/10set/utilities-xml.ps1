...
################################################################################
  ...
  {
    Write-Error "${ERROR_LABEL} [$(${MyInvocation}.MyCommand.Name)] Failed to create XmlReader for [${FileType}] file [$(Sanitize-Path -Path ${originalXmlFile})] due to [$(${_}.GetType().FullName)]: [$(${_}.Exception.Message)]."
    return 1
  }
...
${stat} = Validate-XML-File -XmlFile /home/banerkx/git/mockito-cookbook/xsd/10set/example_10.xml -Schema /home/banerkx/git/mockito-cookbook/xsd/10set/example_10.xsd -FileType 'example_10'
exit ${stat}
